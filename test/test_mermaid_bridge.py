...
class TestParseMermaid:
    def test_simple_nodes(self):
        nodes, edges, clusters, top_nodes, _, _, _ = _parse_mermaid(_SIMPLE)
        assert "A" in nodes
        assert "B" in nodes
    ...
    def test_no_clusters(self):
        _, _, clusters, _, _, _, _ = _parse_mermaid(_SIMPLE)
        assert clusters == []

    def test_top_nodes_not_in_cluster(self):
        _, _, _, top_nodes, _, _, _ = _parse_mermaid(_SIMPLE)
        assert "A" in top_nodes
        assert "B" in top_nodes

    def test_cluster_extracted(self):
        _, _, clusters, _, _, _, _ = _parse_mermaid(_WITH_CLUSTER)
    ...
    def test_top_node_not_in_cluster(self):
        _, _, clusters, top_nodes, _, _, _ = _parse_mermaid(_WITH_CLUSTER)
        assert "top" in top_nodes
        assert "top" not in clusters[0].nodes

    def test_nested_clusters(self):
        _, _, clusters, _, _, _, _ = _parse_mermaid(_NESTED)
        assert len(clusters) == 1
        outer = clusters[0]
    ...
    def test_example_mmd_nodes(self):
        ...
        nodes, edges, clusters, top_nodes, _, _, _ = _parse_mermaid(text)
        assert "gcp_lb" in top_nodes
        assert nodes["gcp_lb"] == "GCP LB"

    def test_example_mmd_edges(self):
        text = EXAMPLE_MMD.read_text()
        _, edges, _, _, _, _, _ = _parse_mermaid(text)
    ...
    def test_example_mmd_clusters(self):
        text = EXAMPLE_MMD.read_text()
        _, _, clusters, _, _, _, _ = _parse_mermaid(text)
...
class TestBuildDot:
    def _dot_for(self, text):
        nodes, edges, clusters, top_nodes, node_colors, node_strokes, node_shapes = _parse_mermaid(text)
        return _build_dot(nodes, edges, clusters, top_nodes, node_colors, node_strokes, node_shapes)
